<template>
  <div class="color-page">
    <div class="color-block-item" v-for="item in colorList">
      <div class="header">
        <p class="color-block-item-title">{{ colorThemes[item].title }}</p>
      </div>
      <div v-for="v in colorThemes[item].list" class="color-item" :style="`background-color:${v.value}`">
...
const colorList: string[] = ["primary", "success", "warning", "danger", "gary"];
...
const colorThemes = {
  ...
  gary: {
    ...
    list: [
      ...
      { name: "gary-6", value: theme["gary-6"], color: theme["gary-1"] },
